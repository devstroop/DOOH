...
namespace DOOH.Client
{
    public partial class DOOHDBService
    {
        ...
        public async System.Threading.Tasks.Task ExportAdboardTokensToExcel(Query query = null, string fileName = null)
        {
        ...
        public async System.Threading.Tasks.Task ExportAdboardTokensToCSV(Query query = null, string fileName = null)
        {
        ...
        partial void OnGetAdboardTokens(HttpRequestMessage requestMessage);

        public async Task<Radzen.ODataServiceResult<DOOH.Server.Models.DOOHDB.AdboardToken>> GetAdboardTokens(Query query)
        {
            return await GetAdboardTokens(filter:$"{query.Filter}", orderby:$"{query.OrderBy}", top:query.Top, skip:query.Skip, count:query.Top != null && query.Skip != null);
        }

        public async Task<Radzen.ODataServiceResult<DOOH.Server.Models.DOOHDB.AdboardToken>> GetAdboardTokens(string filter = default(string), string orderby = default(string), string expand = default(string), int? top = default(int?), int? skip = default(int?), bool? count = default(bool?), string format = default(string), string select = default(string))
        {
            var uri = new Uri(baseUri, $"AdboardTokens");
            uri = Radzen.ODataExtensions.GetODataUri(uri: uri, filter:filter, top:top, skip:skip, orderby:orderby, expand:expand, select:select, count:count);

            var httpRequestMessage = new HttpRequestMessage(HttpMethod.Get, uri);

            OnGetAdboardTokens(httpRequestMessage);

            var response = await httpClient.SendAsync(httpRequestMessage);

            return await Radzen.HttpResponseMessageExtensions.ReadAsync<Radzen.ODataServiceResult<DOOH.Server.Models.DOOHDB.AdboardToken>>(response);
        }

        partial void OnCreateAdboardToken(HttpRequestMessage requestMessage);

        public async Task<DOOH.Server.Models.DOOHDB.AdboardToken> CreateAdboardToken(DOOH.Server.Models.DOOHDB.AdboardToken adboardToken = default(DOOH.Server.Models.DOOHDB.AdboardToken))
        {
            var uri = new Uri(baseUri, $"AdboardTokens");

            var httpRequestMessage = new HttpRequestMessage(HttpMethod.Post, uri);

            httpRequestMessage.Content = new StringContent(Radzen.ODataJsonSerializer.Serialize(adboardToken), Encoding.UTF8, "application/json");

            OnCreateAdboardToken(httpRequestMessage);

            var response = await httpClient.SendAsync(httpRequestMessage);

            return await Radzen.HttpResponseMessageExtensions.ReadAsync<DOOH.Server.Models.DOOHDB.AdboardToken>(response);
        }

        partial void OnDeleteAdboardToken(HttpRequestMessage requestMessage);

        public async Task<HttpResponseMessage> DeleteAdboardToken(int adboardId = default(int))
        {
            var uri = new Uri(baseUri, $"AdboardTokens({adboardId})");

            var httpRequestMessage = new HttpRequestMessage(HttpMethod.Delete, uri);

            OnDeleteAdboardToken(httpRequestMessage);

            return await httpClient.SendAsync(httpRequestMessage);
        }

        partial void OnGetAdboardTokenByAdboardId(HttpRequestMessage requestMessage);

        public async Task<DOOH.Server.Models.DOOHDB.AdboardToken> GetAdboardTokenByAdboardId(string expand = default(string), int adboardId = default(int))
        {
            var uri = new Uri(baseUri, $"AdboardTokens({adboardId})");
            ...
            OnGetAdboardTokenByAdboardId(httpRequestMessage);
        ...
        partial void OnUpdateAdboardToken(HttpRequestMessage requestMessage);
        
        public async Task<HttpResponseMessage> UpdateAdboardToken(int adboardId = default(int), DOOH.Server.Models.DOOHDB.AdboardToken adboardToken = default(DOOH.Server.Models.DOOHDB.AdboardToken))
        {
            var uri = new Uri(baseUri, $"AdboardTokens({adboardId})");

            var httpRequestMessage = new HttpRequestMessage(HttpMethod.Patch, uri);

            httpRequestMessage.Headers.Add("If-Match", adboardToken.ETag);    

            httpRequestMessage.Content = new StringContent(Radzen.ODataJsonSerializer.Serialize(adboardToken), Encoding.UTF8, "application/json");

            OnUpdateAdboardToken(httpRequestMessage);
